...
using Youbetme.DataObjects.Validation;
...
namespace FormsSample
{
	public class App
	{
		...
		{	
			// Initialization 
			Content.Init ("http://dev.youbetme.com/cdn");
			...
			SuperProxy.Init ("https://dev.youbetme.com/svc/service.asmx", "api-token-here", "api-secret-here");
		...
		static void GetBetEnabledLeaguesAsync()
		{
			var leagueFilter = new LeagueFilter ();
			leagueFilter.BetEnabled = true;
			// TODO: Add the lines below if you want to fetch bet enabled 
			leagueFilter.WithEvents = new SportEventFilter ();
			// TODO: Add the lines below if you want to fetch only future events
			leagueFilter.WithEvents.MinGameDate = DateTime.UtcNow;
			// TODO: Add the lines below if you want to fetch only future events
			leagueFilter.WithEvents.WithOdds = true;

			SportEventProxy.GetLeaguesCompleted += HandleGetLeaguesCompleted;
			SportEventProxy.GetLeaguesAsync (Token, leagueFilter, null);
		}

		static void GetScoresEnabledLeaguesAsync()
		{
			var leagueFilter = new LeagueFilter ();
			leagueFilter.ScoresEnabled = true;
			// TODO: Add the lines below if you want to fetch bet enabled 
			leagueFilter.WithEvents = new SportEventFilter ();
			// TODO: Set past date below to get past events with scores history
			leagueFilter.WithEvents.MinGameDate = DateTime.UtcNow.AddDays (-30);

			SportEventProxy.GetLeaguesCompleted += HandleGetLeaguesCompleted;
			SportEventProxy.GetLeaguesAsync (Token, leagueFilter, null);
		}

		static void HandleGetLeaguesCompleted (Response<List<League>> response, object asyncState = null)
		{
			if (response.Status != Status.Success) 
			{
				// TODO: Display response.Message
		...
		static void GetSportEventsAsync()
		{
			var filter = new SportEventFilter ();
			// TODO: Provide league id below
			filter.LeagueId = "5321b3bfc2f1cf12e4deaae0"; //"league-id-here"; // MLB : 5321b3bfc2f1cf12e4deaae0
			// TODO: Add the line below to retrieve live events
			filter.IsLiveEvent = true;
			filter.MinGameDate = DateTime.UtcNow;
			filter.WithOdds = true;

			SportEventProxy.GetEventsCompleted += HandleGetEventsCompleted;
			SportEventProxy.GetEventsAsync (Token, filter, null);
		}

		static void HandleGetEventsCompleted (Response<List<SportEvent>> response, object asyncState = null)
		{
			if (response.Status != Status.Success) 
			{
				// TODO: Display response.Message
		...
		static void CreateSportBet()
		{
			var betLine = new SportBetLine ();
			// TODO: Specify the bet line type below
			// BetLineType.HomeWin - if the users chose home team to win (moneyline)
			// BetLineType.AwayWin - if the users chose away team to win (moneyline)
			// BetLineType.Over - if the users chose total score to be over bet line total value
			// BetLineType.Under - if the users chose total score to be under bet line total value
			// BetLineType.HomeOU - if the users chose home team to win (point spread)
			// BetLineType.AwayOU - if the users chose away team to win (point spread)
			// Note: Useful reference for betting options can be found below:
			// http://www.explainbettingodds.com/moneyline.html
			// http://www.explainbettingodds.com/point-spread.html
			betLine.Type = BetLineType.HomeWin;
			// TODO: Provide below sport event Id
			betLine.SportEventId = "sport-event-id-here";
			// TODO: Provide below the home team name
			betLine.HomeTeam = "home-team-name-here";
			// TODO: Copy below the sport event home point spread (SportEvent.BettingLine.HomeOU)
			// betLine.HomeOU = 0;
			// TODO: Provide below the away team name
			betLine.AwayTeam = "away-team-name-here";
			// TODO: Copy below the sport event away point spread (SportEvent.BettingLine.AwayOU)
			// betLine.AwayOU = 0;
			// TODO: Copy below the betting line total value (SportEvent.BettingLine.Total)
			// betLine.Total = 0;
			// TODO: Provide below the Donbest event id
			betLine.DonEventId = "don-best-event-id-here";
			// TODO: Provide below sport event game date
			betLine.GameDate = DateTime.UtcNow;
			...
			bet.BetLine = betLine;
			// Build the bet title from home and away teams names
			bet.BetTitle = Bet.BuildBetTitle(betLine.HomeTeam, betLine.AwayTeam);
			// Build the bet terms from the bet line type
			bet.BetTerms = Bet.BuildBetRules(betLine);

			// Specify the bet maker Id
			bet.CustomerId = ContextCustomer.Id;

			// Specify bet type 
			bet.Type = BetType.Sports;
			...
			// Optionally add a teammate
			var teammate = new Member ();
			teammate.CustomerId = "teammate-id-here";
			betMakerTeam.Members.Add (teammate);
			...
			// Add the first opponent
			var opponent1 = new Member ();
			opponent1.CustomerId = "opponent1-id-here";
			opponentTeam.Members.Add (opponent1);

			// Opptionally add another opponent
			var opponent2 = new Member ();
			opponent2.CustomerId = "opponent2-id-here";
			opponentTeam.Members.Add (opponent2);
			...
			// Add as many spectators as needed
			var spectator1 = new Member ();
			spectator1.CustomerId = "spectator-id-here";
			audience.Members.Add (spectator1);
		...
		static void CreatePediction()
		{
			var bet = new Bet ();
			bet.Status = BetStatus.Active;

			bet.BetTitle = "I'll win the Hackathon";

			// Specify the bet maker Id
			bet.CustomerId = ContextCustomer.Id;

			// Specify bet type 
			bet.Type = BetType.Prediction;
			...
			// Add the first opponent
			var opponent1 = new Member ();
			opponent1.CustomerId = Customer.YBMPredictionsUserId;
			opponentTeam.Members.Add (opponent1);
			bet.Teams.Add (opponentTeam);
